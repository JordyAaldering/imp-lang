...
use super::{Id, AstConfig, VarInfo, LocalDef, ScopeBlock, Stmt};
...
impl<'ast, Ast: AstConfig> Fundef<'ast, Ast> {
    ...
    pub fn find_local_def(&self, key: &'ast VarInfo<'ast, Ast>) -> Option<LocalDef<'ast, Ast>> {
        let body_scope = self.scope_block();
        find_local_in_scopes(&vec![body_scope], key)
    }
}

pub fn find_local_in_scopes<'ast, Ast: AstConfig>(scopes: &Vec<ScopeBlock<'ast, Ast>>, key: &'ast VarInfo<'ast, Ast>) -> Option<LocalDef<'ast, Ast>> {
