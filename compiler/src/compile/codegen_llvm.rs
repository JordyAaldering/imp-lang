...
impl CodegenContext {
    ...
    fn compile_id<'ast>(&self, id: ArgOrVar<'ast, TypedAst>, fargs: &[LLVMValueRef], fundef: &Fundef<'ast, TypedAst>) -> LLVMValueRef {
        if let Some(i) = fundef.arg_index(id) {
            return fargs[i];
        }

        match fundef.find_local_def(id.as_local().unwrap()).unwrap() {
            LocalDef::Assign(expr) => self.compile_expr(expr, fargs, fundef),
            LocalDef::IndexRange { .. } => todo!("LLVM codegen for index variables requires loop lowering"),
        }
    }

    pub fn compile_fundef<'ast>(&self, f: &Fundef<'ast, TypedAst>) -> LLVMValueRef {
        unsafe {
    ...
    pub fn compile_expr<'ast>(
        ...
        match expr {
            ...
            Expr::Binary(Binary { l, r, op }) => {
                let l = self.compile_id(*l, fargs, fundef);
                let r = self.compile_id(*r, fargs, fundef);
