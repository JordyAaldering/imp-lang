...
impl CodegenContext {
    ...
    fn compile_fundef(&mut self, fundef: &Fundef<TypedAst>) -> String {
        ...
        let ret_code = match fundef.block.ret {
            ...
            ArgOrVar::Var(k) => self.compile_expr(fundef, &fundef.block.ssa[k]),
    ...
    fn compile_expr(&mut self, fundef: &Fundef<TypedAst>, expr: &Expr<TypedAst>) -> String {
        ...
        match expr {
            Expr::Tensor(Tensor { iv, body: expr, lb, ub }) => {
                ...
                let iv_name = fundef.block.ids[iv.0].name.clone();
                ...
                if let ArgOrVar::Var(k) = expr.ret {
                    let expr_code = self.compile_expr(fundef, &fundef.block.ssa[k]);
                ...
                if let ArgOrVar::Var(k) = ub {
                    let l_code = self.compile_expr(fundef, &fundef.block.ssa[*k]);
                    self.stmts.push(format!("{} {} = {};", to_ctype(&fundef.block.ids[*k].ty), fundef.block.ids[*k].name, l_code));
                }
            ...
            Expr::Binary(Binary { l, r, op }) => {
                ...
                if let ArgOrVar::Var(k) = r {
                    let r_code = self.compile_expr(fundef, &fundef.block.ssa[*k]);
                    self.stmts.push(format!("{} {} = {};", to_ctype(&fundef.block.ids[*k].ty), fundef.block.ids[*k].name, r_code));
                }
